...
- **Scripts that describe how the raw partner submissions became the
  published dataset** — `preprocessing/` and the QC/filtering/imputation
  parts of `gseu_preprocessing/`. The raw, per-partner time series
  themselves are **not** published anywhere (data-sharing constraints, see
  the manuscript), so these scripts cannot be re-run end-to-end. They're
  published so the exact method is inspectable and any question about it
  can be answered from the code, not to be a turnkey pipeline for
...
- `plotting/` — reproduces Fig. 2, 3, 5, 6, 7, and A1 directly from `data/`,
  ...
  code included here, but the underlying cluster analysis was carried out
  by BGS/TNO and isn't reproducible here — that script consumes
  `data/trends_cluster.csv`'s `cluster_number` column, already published.
  Fig. 1 (workflow diagram) and Fig. 4 (example weather-variable plot) are
  illustrative and not generated by code.
- `trends/` — Mann-Kendall trend test with Theil-Sen/seasonal Sen's slope
  ...
  the original methodology code (W.J. Zaadnoordijk, TNO-GDN) behind the
  Dutch Groundwater head viewer (TNO-GDN, 2024) cited in the paper;
  ...
  for most stations — see the comment in `requirements.txt`.
