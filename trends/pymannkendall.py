...
def slope_ci(var_s, slopes, alpha):
    slopes_finite = slopes[~np.isnan(slopes)]
    slopes_finite.sort()
    ppf = norm.ppf(alpha / 2.)
    sigma = np.sqrt(var_s)
    Ru = min(int(np.round((len(slopes_finite) - ppf*sigma)/2.)), len(slopes_finite)-1)
    Rl = max(int(np.round((len(slopes_finite) + ppf*sigma)/2.)) - 1, 0)
    return slopes_finite[[Rl, Ru]]
...
def seasonal_sens_slope(x_old, period=12):
    ...
    --------
      ...
      >>> import pymannkendall as mk
      >>> x = np.random.rand(120)
      >>> slope,intercept,slopes = mk.seasonal_sens_slope(x, 12)
    """
    res = namedtuple('Seasonal_Sens_Slope_Test', ['slope','intercept','slopes'])
...
def original_test(x_old, alpha = 0.05):
    ...
    --------
      ...
      >>> trend,h,p,z,tau,s,var_s,slope,intercept,slope_ci = mk.original_test(x,0.05)
    """
    res = namedtuple('Mann_Kendall_Test', ['trend', 'h', 'p', 'z', 'Tau', 's', 'var_s', 'slope', 'intercept','slope_ci'])
    x, c = __preprocessing(x_old)
    x, n = __missing_values_analysis(x, method = 'skip')
    ...
    slope, intercept, slopes = sens_slope(x_old)
    ci = slope_ci(var_s, slopes, alpha)

    return res(trend, h, p, z, Tau, s, var_s, slope, intercept, ci)
...
def hamed_rao_modification_test(x_old, alpha = 0.05, lag=None):
    ...
    --------
      ...
      >>> import pymannkendall as mk
      >>> x = np.random.rand(1000)
      >>> trend,h,p,z,tau,s,var_s,slope,intercept,slope_ci = mk.hamed_rao_modification_test(x,0.05)
    """
    res = namedtuple('Modified_Mann_Kendall_Test_Hamed_Rao_Approach', ['trend', 'h', 'p', 'z', 'Tau', 's', 'var_s', 'slope', 'intercept', 'slope_ci'])
    x, c = __preprocessing(x_old)
    x, n = __missing_values_analysis(x, method = 'skip')
    ...
    for i in range(1,lag):
        if not (acf_1[i] <= upper_bound and acf_1[i] >= lower_bound):
...
def yue_wang_modification_test(x_old, alpha = 0.05, lag=None):
    ...
    --------
      ...
      >>> import pymannkendall as mk
      >>> x = np.random.rand(1000)
      >>> trend,h,p,z,tau,s,var_s,slope,intercept,slope_ci = mk.yue_wang_modification_test(x,0.05)
    """
    res = namedtuple('Modified_Mann_Kendall_Test_Yue_Wang_Approach', ['trend', 'h', 'p', 'z', 'Tau', 's', 'var_s', 'slope', 'intercept', 'slope_ci'])
    x, c = __preprocessing(x_old)
    x, n = __missing_values_analysis(x, method = 'skip')
...
def pre_whitening_modification_test(x_old, alpha = 0.05):
    ...
    --------
      ...
      >>> import pymannkendall as mk
      >>> x = np.random.rand(1000)
      >>> trend,h,p,z,tau,s,var_s,slope,intercept,slope_ci = mk.pre_whitening_modification_test(x,0.05)
    """
    res = namedtuple('Modified_Mann_Kendall_Test_PreWhitening_Approach', ['trend', 'h', 'p', 'z', 'Tau', 's', 'var_s', 'slope', 'intercept', 'slope_ci'])
...
def trend_free_pre_whitening_modification_test(x_old, alpha = 0.05):
    ...
    --------
      ...
      >>> import pymannkendall as mk
      >>> x = np.random.rand(1000)
      >>> trend,h,p,z,tau,s,var_s,slope,intercept,slope_ci = mk.trend_free_pre_whitening_modification_test(x,0.05)
    """
    res = namedtuple('Modified_Mann_Kendall_Test_Trend_Free_PreWhitening_Approach', ['trend', 'h', 'p', 'z', 'Tau', 's', 'var_s', 'slope', 'intercept', 'slope_ci'])
...
def multivariate_test(x_old, alpha = 0.05):
    ...
    --------
      ...
      >>> import pymannkendall as mk
      >>> x = np.random.rand(1000)
      >>> trend,h,p,z,tau,s,var_s,slope,intercept,slope_ci = mk.multivariate_test(x,0.05)
    """
    res = namedtuple('Multivariate_Mann_Kendall_Test', ['trend', 'h', 'p', 'z', 'Tau', 's', 'var_s', 'slope', 'intercept', 'slope_ci'])
    s = 0
    var_s = 0
    denom = 0
...
#     x, n = __missing_values_analysis(x, method = 'skip')  # It makes all column at the same size
    ...
    slope, intercept, slopes = seasonal_sens_slope(x_old, period = c)
...
def seasonal_test(x_old, period = 12, alpha = 0.05):
    ...
    --------
      ...
      >>> import pymannkendall as mk
      >>> x = np.random.rand(1000)
      >>> trend,h,p,z,tau,s,var_s,slope,intercept,slope_ci = mk.seasonal_test(x,0.05)
    """
    res = namedtuple('Seasonal_Mann_Kendall_Test', ['trend', 'h', 'p', 'z', 'Tau', 's', 'var_s', 'slope', 'intercept', 'slope_ci'])
    x, c = __preprocessing(x_old)
    n = len(x)
    ...
    trend, h, p, z, Tau, s, var_s, slope, intercept, ci = multivariate_test(x, alpha = alpha)
...
def regional_test(x_old, alpha = 0.05):
    ...
    --------
      ...
      >>> trend,h,p,z,tau,s,var_s,slope,intercept,slope_ci = mk.regional_test(x,0.05)
    """
    res = namedtuple('Regional_Mann_Kendall_Test', ['trend', 'h', 'p', 'z', 'Tau', 's', 'var_s', 'slope', 'intercept', 'slope_ci'])
    
    trend, h, p, z, Tau, s, var_s, slope, intercept, ci = multivariate_test(x_old)
...
def correlated_multivariate_test(x_old, alpha = 0.05):
    ...
    --------
      ...
      >>> trend,h,p,z,tau,s,var_s,slope,intercept,slope_ci = mk.correlated_multivariate_test(x,0.05)
    """
    res = namedtuple('Correlated_Multivariate_Mann_Kendall_Test', ['trend', 'h', 'p', 'z', 'Tau', 's', 'var_s', 'slope', 'intercept', 'slope_ci'])
    x, c = __preprocessing(x_old)
    x, n = __missing_values_analysis(x, method = 'skip')
...
def correlated_seasonal_test(x_old, period = 12 ,alpha = 0.05):
    ...
    --------
      ...
      >>> import pymannkendall as mk
      >>> x = np.random.rand(1000)
      >>> trend,h,p,z,tau,s,var_s,slope,intercept,slope_ci = mk.correlated_seasonal_test(x,0.05)
    """
    res = namedtuple('Correlated_Seasonal_Mann_Kendall_test', ['trend', 'h', 'p', 'z', 'Tau', 's', 'var_s', 'slope', 'intercept', 'slope_ci'])
    x, c = __preprocessing(x_old)
    ...
    trend, h, p, z, Tau, s, var_s, slope, intercept, ci = correlated_multivariate_test(x)
...
def partial_test(x_old, alpha = 0.05):
    ...
    --------
      ...
      >>> import pymannkendall as mk
      >>> x = np.random.rand(1000, 2)
      >>> trend,h,p,z,tau,s,var_s,slope,intercept,slope_ci = mk.partial_test(x,0.05)
    """
    res = namedtuple('Partial_Mann_Kendall_Test', ['trend', 'h', 'p', 'z', 'Tau', 's', 'var_s', 'slope', 'intercept', 'slope_ci'])
    ...
    p, h, trend = __p_value(z, alpha)
    slope, intercept, slopes = sens_slope(x_old[:,0])
